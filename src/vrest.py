...
class ServerParams():
    def __init__(self):
        ...
        self.base_request_string = ("https://%s:%s/api" % (self.server_ip, self.server_port))
        if getattr(sys, 'frozen', False):
            application_path = os.path.dirname(sys.executable)
        elif __file__:
            application_path = os.path.dirname(__file__)
        self.cert_path = application_path + "\\hacktux-data\\https\\hacktux_cert.crt"
...
def check_response(response):
    '''
    sends the data from the request
    '''
    return {
...
def authenticate(user, password):
    ...
    Returns status code.
    '''
    authentication_string = base64.b64encode((user + ":" + password).encode('utf-8'))
    sp.headers["Authorization"] = ("Basic %s" % authentication_string.decode('utf-8'))
    
    response = requests.get(sp.base_request_string + "/vms", \
        headers=sp.headers, verify=sp.cert_path)
...
def update_vmnet_mactoip(vmnet, mac, params):
    ...
    response = requests.put(sp.base_request_string + "/vmnet/" + vmnet + "/mactoip/" + mac, \
        headers=sp.headers, json=params,verify=sp.cert_path)
    return check_response(response)
...
def update_power(vm_id, params):
    ...
    response = requests.put(sp.base_request_string + "/vms/" + vm_id + "/power", \
        headers=sp.headers, data=params,verify=sp.cert_path)
    return check_response(response)
